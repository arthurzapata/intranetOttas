...
export function obtenerOpcionesRegistro() {
  return apiRequest<DocumentoFormOptions>('/etrami/v2/documentos/opciones')
}
export function obtenerCargos(instanciaId: number) {
  return apiRequest<CargoOption[]>(`/etrami/v2/documentos/instancias/${instanciaId}/cargos`)
}
export function registrarDocumento(data: FormData) {
  return apiRequest<{ id: number; message?: string }>('/etrami/v2/documentos', { method:'POST', body:data })
}
export function buscarReferencias(criterio: string) {
  return apiRequest<Array<{ id:number; text:string }>>(`/etrami/v2/documentos/referencias?criterio=${encodeURIComponent(criterio)}`)
}
export function listarAnulados(filters: { criterio:string; tipo_documento_id:number; estado:string; page:number }) {
  ...
  return apiRequest<AnuladosResponse>(`/etrami/v2/documentos/anulados-rechazados?${query}`)
}
export function restaurarDocumento(tipo:'anulado'|'rechazado', id:number) {
  return apiRequest<void>(`/etrami/v2/documentos/${tipo}/${id}/restaurar`, { method:'POST', body:JSON.stringify({}) })
}
